...
<template>
  ...
  <VCard rounded="xl" variant="flat"><VTable><thead><tr><th>User</th><th>Type</th><th>Message</th><th>Created</th><th /></tr></thead><tbody><tr v-for="item in store.notifications" :key="item.id"><td>{{ item.user_id }}</td><td><VChip size="small">{{ item.type }}</VChip></td><td>{{ String(item.message.title ?? item.message.body ?? '') }}</td><td>{{ item.created_at }}</td><td><VBtn icon="mdi-delete-outline" size="small" variant="text" @click="remove(item.id)" /></td></tr></tbody></VTable></VCard>
</template>
